...
Install the skill from this repo path. Use `--method git` so that both public
and credentialed GitHub access work consistently:
...
retain their old skill cache; you do not need to close unrelated Claude or Codex
sessions.
...
If you only have a paper topic or a broad research direction and do not yet have
a clear research question, the Codex router should start with ARS Socratic
scoping:
...
For review tasks, provide the manuscript or a path to the manuscript, along
with the review mode you want:
...
- Cross-model verification is disabled by default. When explicitly requested in
  ...
  `ANTHROPIC_API_KEY`; the external reviewer uses the Anthropic Claude Opus 4.7
  API, not the Codex/OpenAI API. Upstream GPT/Gemini secondary-dispatch
  instructions are ignored unless this explicit Anthropic configuration is
  present.
...
- If a citation, source, statistic, or journal policy cannot be verified, Codex
  should mark it as unverified rather than fabricating support.
